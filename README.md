...
3. 使用vue-persistedstate进行本地状态持久化存储，编写storage.js进行localstorage加密处理，防止刷新丢失数据并保证数据安全。
...
另外，后端代码涉及数据库账号密码，还有https的证书等暂时不方便公开，但是后端接口是部署好了的，项目文件夹src下的api.js里面
**将localhost替换为域名biubiubius.com即可**，不要瞎搞奥，验证码要钱的咧。
